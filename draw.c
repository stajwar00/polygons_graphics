...
void add_box( struct matrix * edges,
  ...
  add_polygon(edges, x0, y1, z1, x0, y0, z1, x1, y1, z1);
  //left
  add_polygon(edges, x0, y0, z1, x0, y1, z0, x0, y0, z0);
  add_polygon(edges, x0, y0, z1, x0, y1, z1, x0, y1, z0);
  //right
  add_polygon(edges, x1, y0, z0, x1, y1, z0, x1, y0, z1);
...
void add_torus( struct matrix * edges, 
  ...
  for ( lat = latStart; lat < latStop; lat++ ) {
    for ( longt = longStart; longt < longStop; longt++ ) {
      ...
      index = (lat * steps + longt + 1) % points->lastcol;
      x1 = points->m[0][index];
      y1 = points->m[1][index];
      z1 = points->m[2][index];

      index = (lat * steps + longt + steps) % points->lastcol;
      x2 = points->m[0][index];
      y2 = points->m[1][index];
      z2 = points->m[2][index];

      index = (lat * steps + longt + steps + 1) % points->lastcol;
